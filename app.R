...
dino_db <- list(
  ...
  "Baryonyx" = list(
    facts = "A fish-eating predator with strong forelimbs and a long snout, skilled in both land and aquatic hunting.",
    strength = 7, speed = 6, ferocity = 6, intelligence = 5, agility = 5, stamina = 6,
    countries = c("UK", "Spain", "Portugal"),
    image = "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Baryonyx_UDL.png/960px-Baryonyx_UDL.png"
  ),
  ...
  "Mosasaurus" = list(
    facts = "A massive marine reptile with powerful jaws and fins, dominating prehistoric seas with speed and strength.",
    strength = 9, speed = 8, ferocity = 8, intelligence = 5, agility = 6, stamina = 7,
    countries = c("USA", "Netherlands", "Belgium", "Jordan"),
    image = "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b0/Mosasaurus.png/960px-Mosasaurus.png"
  ),
...
server <- function(input, output, session) {
  ...
  output$stats_title <- renderUI({
    dino1_color <- "darkgreen"
    dino2_color <- "purple"

    HTML(paste0(
      "<span style='color:", dino1_color, "; font-weight:bold;'>", input$dino1, "</span>",
      " vs ",
      "<span style='color:", dino2_color, "; font-weight:bold;'>", input$dino2, "</span>: Dinosaur Statistics"
    ))
  })
  ...
  output$map_title <- renderUI({
    ...
    HTML(paste0(
      ...
      "Countries Where <span style='color:", dino1_color, ";'>", input$dino1, "</span>",
      " and <span style='color:", dino2_color, ";'>", input$dino2, "</span> Were Found",
      "</div>",
      "<div style='font-style: italic; font-size: 12px; margin-top: 0px;'>",
      "(Countries where both were found are in brown)",
      "</div>",
      "</div>"
    ))
  })
  ...
  output$dino_info <- renderUI({
    ...
    tagList(
      tags$div(style = "border-right: 20px solid darkgreen; background-color: #e6f4ea; padding-left: 10px; margin-bottom: 20px;",
               h4(input$dino1),
               tags$div(style = "position: relative; display: inline-block; width: 100%;",
                        tags$img(src = dino_db[[input$dino1]]$image,
                                 style = paste0("max-height:",max_height,"px; width: 100%; object-fit: contain; display: block;")),
